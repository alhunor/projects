...
public:
     ...
     */
    bool RemovePathAndChildren(const std::string& path);

    /**
     * Returns the number of recursively watched paths.
     */
    int GetNumberOfWatchedPaths() {return watchedPaths.size();}
     ...
     */
    int GetNumberOfChangedPaths() { autoLocker lock; return m_changedPaths.size(); }

    /**
     * Returns the list of paths which maybe got changed, i.e., for which
     * a change notification was received.
     */
    std::vector<std::string> GetChangedPaths() {autoLocker lock; return m_changedPaths; }

     // Clears the list of changed paths
    void ClearChangedPaths() { autoLocker lock; m_changedPaths.clear(); }
...
private:
	...
	std::vector<std::string>           watchedPaths;   ///< list of watched paths.
	std::vector<std::string>           m_changedPaths; ///< list of paths which got changed
    ...
    public:
        CDirWatchInfo(CAutoFile && hDir, const std::string& DirectoryName);
        ~CDirWatchInfo();
    ...
    public:
        ...
        std::string   m_DirName;      ///< the directory that we're watching
        CHAR        m_Buffer[READ_DIR_CHANGE_BUFFER_SIZE]; ///< buffer for ReadDirectoryChangesW
        OVERLAPPED  m_Overlapped;
        std::string     m_DirPath;      ///< the directory name we're watching with a backslash at the end

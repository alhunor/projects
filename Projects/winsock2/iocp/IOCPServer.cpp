/*
This is a sample code that demonstrates  the use of the I/O Completion ports with WinSock.

* The server will create IOCP, Worker threads, all incoming client sockets will be associated with IOCP,
the server will accept the client sent  message will display it and then send the same message back as an acknowledgement.

Author: Swarajya Pendharkar 10th March 2006
*/
...
//For this simple implementation we can use global variable as well.
//Wanted to demonstrate use of event for shutdown
...
DWORD WINAPI AcceptThread(LPVOID lParam)
{
	...
	//Accept thread will look for accept event, until a Shutdown event is Signaled.

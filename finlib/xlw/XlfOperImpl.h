...
// $Id: XlfOperImpl.h 1407 2012-05-30 12:59:29Z adcockj $
...
namespace xlw { namespace impl {
    ...
    public:
        ...
        //! MyMatrix ctor.
        XlfOper(const MyMatrix& matrix) : lpxloper_(TempMemory::GetMemory<OperType>())
        {
		...
		XlfOper(const HuMatrix<int>& matrix)  : lpxloper_(TempMemory::GetMemory<OperType>())
        {
            int nbRows = matrix.size1();
            int nbCols = matrix.size2();;
            ...
            for (int row(0); row < nbRows; ++row)
            {
                ...
                {
                    LPOPER_TYPE elementOper = OperProps::getElement(lpxloper_, row, col);
                    OperProps::setDouble(elementOper, matrix(row, col));
                }
            }
...
MyStringArray AsStringArray(const char* ErrorId = 0, XlfOperImpl::DoubleVectorConvPolicy policy = XlfOperImpl::UniDimensional) const
{
	MultiRowType nbRows(OperProps::getRows(lpxloper_));
	MultiColType nbCols(OperProps::getCols(lpxloper_));
	...
	for(MultiRowType row(0); row < nbRows; ++row)
	{
		for(MultiRowType col(0); col < nbCols; ++col)
		{
			...
			if(policy == XlfOperImpl::RowMajor)
			{
				result[row * nbCols + col] = element.AsString(ErrorId);
			}
			else
			{
				result[col * nbRows + row] = element.AsString(ErrorId);
			}
		}
	}
	return result;
		...
		gsl_matrix* AsGSLMatrix(const char* ErrorId = 0) const
		{
			MultiRowType nbRows(OperProps::getRows(lpxloper_));
			MultiColType nbCols(OperProps::getCols(lpxloper_));
			gsl_matrix* result = gsl_matrix_calloc(nbRows, nbCols);
			for(MultiRowType row(0); row < nbRows; ++row)
			{
				for(MultiRowType col(0); col < nbCols; ++col)
				{
					XlfOper<LPOPER_TYPE> element(OperProps::getElement(lpxloper_, row, col));
					gsl_matrix_set(result,row, col,element.AsDouble(ErrorId));
				}
			}
			return result;
		} // gsl_matrix* AsGSLMatrix(..) const


	    gsl_vector* AsGSLVector(const char* ErrorId = 0) const
        {
            MultiRowType nbRows(OperProps::getRows(lpxloper_));
            MultiColType nbCols(OperProps::getCols(lpxloper_));

            if (nbRows != 1 && nbCols != 1 || nbRows*nbCols == 0)
            {
                // not a vector we return a failure
                throw "Data is not a Vector";
            }

            gsl_vector* result = gsl_vector_calloc(nbRows * nbCols);

			if (nbRows == 1)
			{
				for(MultiRowType col(0); col < nbCols; ++col)
				{
					XlfOper<LPOPER_TYPE> element(OperProps::getElement(lpxloper_, 0, col));
					result->data[col] = element.AsDouble(ErrorId);
				}
			} else
			{
				for(MultiRowType row(0); row < nbRows; ++row)
				{
					XlfOper<LPOPER_TYPE> element(OperProps::getElement(lpxloper_, row, 0));
					result->data[row] = element.AsDouble(ErrorId);
				}
			} //  else of if (nbRows == 1)

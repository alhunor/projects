...
// For efficiency, a variety of data structures are allocated in bulk.  The following constants determine how many of each structure is allocated  at once.
...
//  A line is considered "nonempty" if it contains something that looks like  a number.  Comments (prefaced by `#') are ignored.
#ifndef TRILIBRARY
...
//  Jumps past the current field by searching for whitespace, then jumps past the whitespace to find the next field.
#ifndef TRILIBRARY
...
// The triangle's origin vertex. The triangle's destination vertex. The triangle's apex vertex. The area of the triangle.
{
  REAL dxoa, dxda, dxod;
  REAL dyoa, dyda, dyod;
...
void mergehulls(struct mesh *m, struct behavior *b, struct otri *farleft,
                struct otri *innerleft, struct otri *innerright, struct otri *farright, int axis)
{
...
void divconqrecurse(struct mesh *m, struct behavior *b, vertex *sortarray,
                    int vertices, int axis, struct otri *farleft, struct otri *farright)
{
...
struct splaynode *circletopinsert(struct mesh *m, struct behavior *b, struct splaynode *splayroot,
...
void triangulate(char *triswitches, struct triangulateio *in, struct triangulateio *out, struct triangulateio *vorout)
{
  ...
  // The -c switch (convex switch) causes a PSLG to be written even if none was read.
  if (b.poly || b.convex)
  {
  ...
  if (b.voronoi)
  {
    writevoronoi(&m, &b, &vorout->pointlist, &vorout->pointattributelist, &vorout->pointmarkerlist, &vorout->edgelist, &vorout->edgemarkerlist, &vorout->normlist);
  }
  if (b.neighbors) {

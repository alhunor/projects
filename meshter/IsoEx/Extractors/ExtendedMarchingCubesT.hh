...
void extended_marching_cubes(const Grid&  _grid,
			     ...
			     double       _feature_angle)
{
  ExtendedMarchingCubesT<Mesh> emc(_grid, _mesh, _feature_angle);
}
...
ExtendedMarchingCubesT<Mesh>::
	ExtendedMarchingCubesT(const Grid&  _grid,
	Mesh&        _mesh,
	double       _feature_angle) : grid_(_grid),     mesh_(_mesh), feature_angle_(_feature_angle / 180.0 * M_PI), n_edges_(0), n_corners_(0)
{
...
template <class Mesh>
void ExtendedMarchingCubesT<Mesh>::process_cube(CubeIdx _idx)
{
  PointIdx           corner[8];
